...
from .models import Property, CommunityPost, Category, Bidding, AppUser, PropertyType, PropertyVisits, Message, \
    GroupChat, Institute, Payment
...
def payment_history(request):
    ...
    types = PropertyType.objects.all()  # Fetch all categories from database
    if property_type and property_type != 'all':
        payments = (payments.filter(property__property_type=property_type.lower()))


    # if bidding_status and bidding_status != 'all':
    #     properties = [prop for prop in properties if prop.bidding_status == bidding_status]

    return render(request, 'mainapp/payments.html', {
        'payments': payments,
        'types': types,
        'bidding_statuses': ['pending', 'accepted', 'rejected'],
        # 'bidding_status': bidding_status,
